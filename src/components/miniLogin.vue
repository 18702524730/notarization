      ...
      <div v-if="regVisible&&wechatcode">
        <form autocomplete="off">
          <div class="ipt-wrap" :class="{inputfocus:regData.phone ||　phoneipt}" @click="phonereg()">
            <input  autocomplete="off" name="username" type="text" class="ipt" v-model="regData.phone" maxlength="11" ref="phonereg" @focus="phoneipt=true" @blur="phoneipt=false">
            <label class="name label-name">请输入手机号</label>
          </div>
          <div class="ipt-wrap code-wrap" :class="{inputfocus:regData.token || capipt}" @click="capreg()">
            <input  autocomplete="off" type="text" class="ipt" v-model="regData.token" maxlength="6" ref="capreg" @focus="capipt=true" @blur="capipt=false">
          ...
          <div class="ipt-wrap password-wrap" :class="{inputfocus:passwordOrigin || passipt}" @click="passreg()">
            <input autocomplete="new-password" name="password" :type="isPasswordType?'password':'text'" class="ipt" v-model="passwordOrigin" maxlength="16" ref="passreg" @focus="passipt=true" @blur="passipt=false">
...
#miniloginwrap{position: fixed;top: 0;left:0;z-index:3000;width: 100%;height: 100%;}
