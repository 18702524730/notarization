  ...
  export default {
    ...
    methods:{
      ...
      getCode(){
        ...
        self.$http.post(self.czUrl+'/api/login/sendPhoneCode', {phone: self.regData.phone})
        .then((response) => {
      ...
      preLogin(){
        ...
        self.$http.post(self.czUrl+'/api/beforeLogin', {
      ...
      login(){
        ...
        var postData = self.regData,
          data = {};
          console.log(self.$cookie.get('resourceType'))
        if (self.showCode) {
        ...
        self.$http.post(self.czUrl+'/api/login', data)
        .then((response) => {
          self.submitDisabled = false;
