  ...
  data () {
    ...
    return {
      ...
      rules:{
        //境外个体
        name_en:[
          { required: true, message: '请输入申请人英文姓名', trigger: 'blur' },
          { min: 2, max: 20, message: '姓名长度 2到 20个字符', trigger: 'blur' }
        ],
        apply_name:[
          { required: true, message: '请输入申请人中文姓名', trigger: 'blur' },
          { min: 2, max: 20, message: '姓名长度 2到 20个字符', trigger: 'blur' }
        ],
        apply_number: [
    ...
    add(formName){
      ...
      self.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          //添加 401错误 页面登录过期提醒
          if(data.status == 401 ) {
            self.$confirm('登录已失效,请重新登录',{
              type: 'warning',
              confirmButtonText: '重新登录',
              closeOnclickModek: false
            }).then(()=> {

              //未登录的事件  向sebeApplysubject.vue传
              self.$emit('getIsLogin');
          }).catch(()=>{
            })};
